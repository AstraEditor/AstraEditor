...
Blockly.BlockSvg.prototype.updateColour = function () {
  ...
  var renderShadowed = this.isShadow() && !Blockly.scratchBlocksUtils.isShadowArgumentReporter(this);
...
Blockly.BlockSvg.prototype.render = function (opt_bubble) {
  ...
  if (this.isScratchExtension && this.outputConnection) {
    cursorX += this.RTL ? -Blockly.BlockSvg.GRID_UNIT : Blockly.BlockSvg.GRID_UNIT;
  }
...
Blockly.BlockSvg.prototype.renderCompute_ = function (iconWidth) {
  ...
  for (var i = 0, input; (input = inputList[i]); i++) {
    ...
    var isSecondInputOnProcedure =
      this.type == 'procedures_definition' && lastType && lastType == Blockly.NEXT_STATEMENT;
    var row;
    ...
    if (
      ...
      (!lastType || lastType == Blockly.NEXT_STATEMENT || input.type == Blockly.NEXT_STATEMENT)
    ) {
      lastType = input.type;
  ...
  inputRows.statementEdge = Blockly.BlockSvg.STATEMENT_INPUT_EDGE_WIDTH + fieldStatementWidth;
...
Blockly.BlockSvg.prototype.computeRightEdge_ = function (curEdge, hasStatement) {
  ...
  if (this.previousConnection || this.nextConnection) {
    ...
    if (this.isShadow() && !Blockly.scratchBlocksUtils.isShadowArgumentReporter(this)) {
      // Single-fields
...
Blockly.BlockSvg.prototype.renderDrawBottom_ = function (steps, cursorY) {
  ...
  if (this.nextConnection) {
    ...
    var notchStart =
      Blockly.BlockSvg.NOTCH_WIDTH + Blockly.BlockSvg.NOTCH_START_PADDING + Blockly.BlockSvg.CORNER_RADIUS;
...
Blockly.BlockSvg.drawStatementInputTop_ = function (steps, cursorX) {
  steps.push(Blockly.BlockSvg.BOTTOM_RIGHT_CORNER);
  steps.push('H', cursorX + Blockly.BlockSvg.STATEMENT_INPUT_INNER_SPACE + 2 * Blockly.BlockSvg.CORNER_RADIUS);
  steps.push(Blockly.BlockSvg.NOTCH_PATH_RIGHT);
...
Blockly.BlockSvg.prototype.renderDefineBlock_ = function (steps, inputRows, input, row, cursorY) {
  ...
  if (input.connection && input.connection.targetBlock()) {
    ...
    rightSide =
      Math.max(Blockly.BlockSvg.MIN_BLOCK_X_WITH_STATEMENT, rightSide) + Blockly.BlockSvg.DEFINE_BLOCK_PADDING_RIGHT;
  }
