...
Blockly.Connection.prototype.canConnectWithReason_ = function (target) {
  ...
  if (blockA && blockA == blockB) {
    ...
    (blockB.type == Blockly.PROCEDURES_PROTOTYPE_BLOCK_TYPE && blockA.type != Blockly.PROCEDURES_DEFINITION_BLOCK_TYPE)
  ) {
...
Blockly.Connection.prototype.canConnectToPrevious_ = function (candidate) {
  ...
  if (isNextConnection || (isFirstStatementConnection && !sourceHasPreviousConn)) {
...
Blockly.Connection.prototype.isConnectionAllowed = function (candidate) {
  ...
  switch (candidate.type) {
    ...
    case Blockly.INPUT_VALUE: {
      ...
      if (candidate.targetConnection && !candidate.targetBlock().isMovable() && !candidate.targetBlock().isShadow()) {
        return false;
      }
      break;
    }
