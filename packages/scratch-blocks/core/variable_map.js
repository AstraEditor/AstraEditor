  ...
  if (variable) {
    ...
    for (var i = 0, block; (block = uses[i]); i++) {
      if (block.type == Blockly.PROCEDURES_DEFINITION_BLOCK_TYPE || block.type == 'procedures_defreturn') {
        ...
        var deleteText = Blockly.Msg.CANNOT_DELETE_VARIABLE_PROCEDURE.replace('%1', variableName).replace(
          '%2',
          procedureName
        );
    ...
    if (uses.length > 1) {
      ...
      var confirmText = Blockly.Msg.DELETE_VARIABLE_CONFIRMATION.replace('%1', String(uses.length)).replace(
        '%2',
        variableName
      );
