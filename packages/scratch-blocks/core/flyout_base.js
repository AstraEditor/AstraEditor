...
Blockly.Flyout.prototype.onResizeHandleMouseMove_ = function (e) {
  ...
  if (!this.resizeAnimationFrame_) {
    this.resizeAnimationFrame_ = requestAnimationFrame(
      function () {
  ...
  for (var i = 0; i < this.buttons_.length; i++) {
    if (this.buttons_[i].getIsCategoryLabel()) {
      ...
      this.categoryScrollPositions.push({
        ...
        position: this.horizontalLayout_ ? categoryLabel.getPosition().x : categoryLabel.getPosition().y
      });
    }
  }
...
Blockly.Flyout.prototype.clearOldBlocks_ = function () {
  ...
  for (var i = 0, block; (block = oldBlocks[i]); i++) {
    if (block.workspace == this.workspace_) {
      if (this.recyclingEnabled_ && Blockly.scratchBlocksUtils.blockIsRecyclable(block)) {
