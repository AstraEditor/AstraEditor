...
Blockly.FieldVariable.prototype.initFlyoutBroadcast_ = function (workspace) {
  ...
  if (workspace.isFlyout && this.defaultType_ == broadcastMsgType && broadcastVars.length != 0) {
...
Blockly.FieldVariable.prototype.getVariableTypes_ = function () {
  ...
  if (variableTypes.length == 0) {
    ...
    throw new Error("'variableTypes' of field variable " + name + ' was an empty list');
  }
...
Blockly.FieldVariable.dropdownCreate = function () {
  ...
  if (this.defaultType_ == Blockly.BROADCAST_MESSAGE_VARIABLE_TYPE) {
    ...
    if (deleteText) {
      options.push([deleteText.replace('%1', name), Blockly.DELETE_VARIABLE_ID]);
    }
  }
