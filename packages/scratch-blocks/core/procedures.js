...
Blockly.Procedures.flyoutCategory = function (workspace) {
  ...
  var showReturn = Blockly.Procedures.DEFAULT_ENABLE_RETURNS ? mutations.length > 0 : workspace.procedureReturnsEnabled;
  if (showReturn) {
...
Blockly.Procedures.mutateCallersAndPrototype = function (name, ws, mutation) {
  ...
  if (defineBlock && prototypeBlock) {
    var callers = Blockly.Procedures.getCallers(name, defineBlock.workspace, defineBlock, true /* allowRecursive */);
...
Blockly.Procedures.editProcedureCallback_ = function (block) {
  ...
  if (block.type == Blockly.PROCEDURES_DEFINITION_BLOCK_TYPE) {
    ...
    if (!innerBlock || !innerBlock.type == Blockly.PROCEDURES_PROTOTYPE_BLOCK_TYPE) {
...
Blockly.Procedures.showProcedureDefCallback_ = function (block) {
  alert('TODO(#1136): implement showing procedure definition (procCode was "' + block.procCode_ + '")');
};
