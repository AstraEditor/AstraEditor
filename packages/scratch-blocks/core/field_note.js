...
Blockly.FieldNote.prototype.showEditor_ = function () {
  ...
  this.fieldEditorWidth_ =
    Blockly.FieldNote.NUM_WHITE_KEYS * Blockly.FieldNote.WHITE_KEY_WIDTH + Blockly.FieldNote.EDGE_PADDING;
  this.fieldEditorHeight_ =
    Blockly.FieldNote.TOP_MENU_HEIGHT + Blockly.FieldNote.WHITE_KEY_HEIGHT + Blockly.FieldNote.EDGE_PADDING;
  ...
  var highCX = lowCX + Blockly.FieldNote.WHITE_KEY_WIDTH * (Blockly.FieldNote.NUM_WHITE_KEYS - 1);
  ...
  this.octaveUpButton = this.addOctaveButton_(
    this.fieldEditorWidth_ + Blockly.FieldNote.INSET * 2 - Blockly.FieldNote.OCTAVE_BUTTON_SIZE,
    false,
    svg
  );

  this.octaveDownMouseDownWrapper_ = Blockly.bindEvent_(this.octaveDownButton, 'mousedown', this, function () {
    this.changeOctaveBy_(-1);
  });
  this.octaveUpMouseDownWrapper_ = Blockly.bindEvent_(this.octaveUpButton, 'mousedown', this, function () {
    this.changeOctaveBy_(1);
  });
...
Blockly.FieldNote.prototype.addPianoOctave_ = function (x, whiteKeyGroup, blackKeyGroup, keySVGarray) {
  ...
  for (var i = 0; i < Blockly.FieldNote.KEY_INFO.length; i++) {
    ...
    if (keySVGarray) {
      ...
      this.mouseDownWrappers_[i] = Blockly.bindEvent_(keySVG, 'mousedown', this, this.onMouseDownOnKey_);
      this.mouseEnterWrappers_[i] = Blockly.bindEvent_(keySVG, 'mouseenter', this, this.onMouseEnter_);
    }
  }
};
...
Blockly.FieldNote.prototype.addCKeyLabel_ = function (x, svg) {
  return Blockly.utils.createSvgElement(
    ...
    {
      x: x,
      y: Blockly.FieldNote.TOP_MENU_HEIGHT + Blockly.FieldNote.WHITE_KEY_HEIGHT - Blockly.FieldNote.KEY_LABEL_PADDING,
...
Blockly.FieldNote.prototype.stepOctaveAnimation_ = function () {
  ...
  this.animationPos_ += (this.animationTarget_ - this.animationPos_) * Blockly.FieldNote.ANIMATION_FRACTION;
