...
const Tags = ({ manifest }) => (
  <span className={styles.tagContainer}>
    ...
    {manifest.tags.includes('not_compatible') && (
      <span className={classNames(styles.tag, styles.tagNotCompatible)}>{settingsTranslations.tagNotCompatible}</span>
    )}
...
const Notice = ({ type, text }) => (
  <div className={styles.notice} type={type}>
    <img className={styles.noticeIcon} src={type === 'conflict' ? alertImage : infoImage} alt="" draggable={false} />
    <div>{text}</div>
  </div>
);
Notice.propTypes = {
...
const Addon = ({
  ...
  const conflictText =
    hasConflict && conflicts && conflicts[id] && conflicts[id].length > 0
      ? settingsTranslations.incompatibleWith.replace(
          '{plugins}',
          conflicts[id].map((cId) => addonTranslations[`${cId}/@name`] || cId).join(', ')
        )
      : '';
    ...
    >
      <div className={styles.addonHeader}>
        <label className={styles.addonTitle}>
          <div className={styles.addonSwitch}>
            <Switch value={settings.enabled || isPending} onChange={handleSwitchChange} />
            {isPending && <span className={styles.pendingIndicator}>{settingsTranslations.pendingIndicator}</span>}
          </div>
      ...
      {expanded && (
        <div className={styles.addonDetails}>
          <div className={styles.description}>{addonTranslations[`${id}/@description`] || manifest.description}</div>
          {manifest.credits && (
          ...
          {manifest.info &&
            manifest.info.map((info) => (
              <Notice key={info.id} type={info.type} text={addonTranslations[`${id}/@info-${info.id}`] || info.text} />
            ))}
          {manifest.noCompiler && <Notice type="warning" text={settingsTranslations.noCompiler} />}
          {incompatibleList && (
            <div className={styles.incompatibleInfo}>
              <span className={styles.incompatibleLabel}>{settingsTranslations.incompatiblePluginsLabel}</span>
              <span className={styles.incompatibleList}>{incompatibleList}</span>
            </div>
          )}
          {isEditorIncompatible && (
            <div className={classNames(styles.incompatibleInfo, styles.editorIncompatible)}>
              ...
              <span className={styles.incompatibleList}>AstraEditor</span>
            </div>
          )}
          {manifest.settings && (
      ...
      {hasConflict && <div className={styles.conflictTextOnRight}>{conflictText}</div>}
    </div>
  );
};
Addon.propTypes = {
      ...
      <div className={styles.addonGroup}>
        ...
        >
          <div className={styles.addonGroupExpandContainer}>
            <img className={styles.addonGroupExpandIcon} src={expandImageBlack} data-open={this.state.open} alt="" />
          </div>
      ...
      if (isEnabled || isPending) {
        ...
        if (conflictingAddons.length > 0) {
          ...
          const activeConflicts = conflictingAddons.filter((cId) => {
            return SettingsStore.getAddonEnabled(cId) || (this.state[cId] && this.state[cId].pendingEnable);
          });
      ...
      try {
        ...
        const handleImportComplete = (e) => {
          ...
          if (failed.length > 0 || pending.length > 0) {
            ...
            if (pending.length > 0) {
              const pendingList = pending
                .map((p) => {
                  ...
                  return settingsTranslations.pendingList.replace('{name}', name).replace('{reason}', reason);
                })
                .join('\n');
            ...
            if (failed.length > 0) {
              const failedList = failed
                .map((f) => {
                  ...
                  return settingsTranslations.importFailed.replace('{name}', name).replace('{reason}', reason);
                })
                .join('\n');
            ...
            alert(settingsTranslations.importSuccess.replace('{count}', successful.length));
          }
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <div className={styles.section}>
            ...
            >
              <span className={styles.feedbackButtonInner}>{settingsTranslations.addonFeedback}</span>
            </a>
          </div>
          {hasConflicts && (
            <ConflictFloatingBanner conflicts={this.state.conflicts} addonTranslations={addonTranslations} />
          )}
          ...
          {!this.state.loading && (
            <div className={styles.section}>
              ...
              <div className={styles.footerButtons}>
                <button className={classNames(styles.button, styles.resetAllButton)} onClick={this.handleResetAll}>
                  {settingsTranslations.resetAll}
                </button>
                <button className={classNames(styles.button, styles.exportButton)} onClick={this.handleExport}>
                  {settingsTranslations.export}
                </button>
                <button className={classNames(styles.button, styles.importButton)} onClick={this.handleImport}>
