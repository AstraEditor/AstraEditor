...
const untilInEditor = () => {
  if (!reduxInstance.state.scratchGui.mode.isPlayerOnly || reduxInstance.state.scratchGui.mode.isEmbedded) {
    return;
  }
    ...
    if (this.manifest.userstyles) {
      for (let i = 0; i < this.manifest.userstyles.length; i++) {
        ...
        const userstyleCondition = () =>
          !this.publicAPI.addon.self.disabled && SettingsStore.evaluateCondition(this.id, userstyle.if);
