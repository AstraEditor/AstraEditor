...
function syncPointerContainerTransform() {
  ...
  const svg = ws.getParentSvg(),
    pc = svg.querySelector(`.${idHead}pointer-container`);
  if (!pc) return;
  const c = ws.getCanvas();
  if (!c) return;
  const t = c.getAttribute('transform');
  if (t) pc.setAttribute('transform', t);
  syncPointerScales();
}
...
function xmlDomToBlockObj(node, blocks, isTop, parentId) {
  ...
  for (const ch of Array.from(node.children || [])) {
    ...
    switch (nm) {
      case 'field': {
        const fn = ch.getAttribute('name'),
          fi = ch.getAttribute('id');
        let tv = '';
        if (ch.childNodes.length > 0) tv = ch.childNodes[0].textContent || '';
        b.fields[fn] = { name: fn, id: fi, value: tv };
        break;
      }
      case 'value':
      case 'statement': {
        const iname = ch.getAttribute('name');
        let cbn = null,
          csn = null;
        for (const gc of Array.from(ch.children || [])) {
          const gn = gc.tagName ? gc.tagName.toLowerCase() : '';
          if (gn === 'block') cbn = gc;
          else if (gn === 'shadow') csn = gc;
        }
        let cb = null,
          cs = null;
        if (!cbn && csn) cbn = csn;
        if (cbn) {
          const co = xmlDomToBlockObj(cbn, blocks, false, id);
          if (co) {
            blocks[co.id] = co;
            cb = co.id;
          }
        ...
        if (csn) {
          const so = xmlDomToBlockObj(csn, blocks, false, id);
          if (so) {
            blocks[so.id] = so;
            cs = so.id;
          }
        }
        b.inputs[iname] = { name: iname, block: cb, shadow: cs };
        break;
      }
      case 'next': {
        let nn = null;
        for (const gc of Array.from(ch.children || [])) if (gc.tagName && gc.tagName.toLowerCase() === 'block') nn = gc;
        if (nn) {
          const no = xmlDomToBlockObj(nn, blocks, false, id);
          if (no) {
            blocks[no.id] = no;
            b.next = no.id;
          }
...
function parseXml(xmlString) {
  ...
  const blocks = {};
  const rn = p.documentElement,
    tn = rn.tagName ? rn.tagName.toLowerCase() : '';
  if (tn === 'block') {
    const o = xmlDomToBlockObj(rn, blocks, true, null);
    if (o) blocks[o.id] = o;
  }
...
function enqueue(op, rtc) {
  ...
  if (!_raf)
    _raf = requestAnimationFrame(() => {
      _raf = null;
      flush(rtc).catch((e) => console.error('[协作] flush 异常:', e));
    });
}

function isProcedureOpcode(opcode) {
  return opcode === 'procedures_definition' || opcode === 'procedures_prototype' || opcode === 'procedures_declaration';
}
...
function lockKey(lockType, lockId) {
  return `${lockType}:${lockId}`;
}
...
function addLockOverlay(svgGroup, userName, noLabel) {
  ...
  // 获取主题色
  const themeColor =
    getComputedStyle(document.documentElement).getPropertyValue('--looks-secondary').trim() || '#855cd6';
...
export function cleanupAllRemote() {
  ...
  if (scaleUpdateRaf) {
    cancelAnimationFrame(scaleUpdateRaf);
    scaleUpdateRaf = null;
  }
  if (toolboxRefreshRaf) {
    cancelAnimationFrame(toolboxRefreshRaf);
    toolboxRefreshRaf = null;
  }
  if (toolboxRefreshTimer) {
    clearTimeout(toolboxRefreshTimer);
    toolboxRefreshTimer = null;
  }
...
function applyRemoteChatBubble(data) {
  ...
  if (text && text.length > 0) {
    ...
    let textWidth = 0;
    try {
      textWidth = nameText.getBBox().width;
    } catch (e) {
      /* 元素未渲染 */
    }
...
function replaceTree(target, xmlString, _rootId, isEditingTarget, oldRootId) {
  ...
  if (ws) {
    ...
    try {
      ...
      if (bn) {
        const px = parseFloat(bn.getAttribute('x')),
          py = parseFloat(bn.getAttribute('y'));
      ...
      console.error('[协作] replaceTree 失败:', e);
    } finally {
      _Blockly.Events.enable();
    }
...
function applyMove(target, op, isEditingTarget) {
  ...
  const ws = getWS();
  if (ws) {
    const blk = ws.getBlockById(op.rootId);
    if (blk) {
      const cp = blk.getRelativeToSurfaceXY();
      blk.moveBy(op.x - cp.x, op.y - cp.y);
    }
  }
}
...
function applyFieldChange(target, op) {
  ...
  for (const fc of op.fields) {
    ...
    target.blocks.changeBlock({
      ...
      element: 'field',
      name: fc.name,
      value: isVar ? fc.id || fc.value : fc.value
    });
    if (ws) {
      const blk = ws.getBlockById(fc.blockId);
      if (blk) {
        try {
          blk.setFieldValue(fc.value, fc.name);
        } catch (e) {}
      }
    }
  }
...
function applyCostumeAdd(target, op) {
  ...
  if (!vm || !op.data) {
    console.warn('[协作-造型] ADD 跳过: vm或data缺失');
    return Promise.resolve();
  }
  const runtime = vm.runtime;
  const storage = runtime && runtime.storage;
  if (!storage) {
    console.warn('[协作] storage 不可用，无法添加造型');
    return Promise.resolve();
  }
  try {
    ...
    console.log(
      `[协作-造型] ADD target=${target.id} index=${op.index} name=${op.name} assetId=${asset.assetId} fmt=${fmt} size=${bytes.byteLength}`
    );
    const costumeObj = {
      name: op.name,
    ...
    return vm
      .addCostume(md5ext, costumeObj, target.id)
      .then(() => {
    ...
    console.error('[协作] 创建造型 asset 失败:', e);
    return Promise.resolve();
  }
}
...
function applyCostumeUpdate(target, op, rtc) {
  ...
  const vm = target.runtime?.vm || window.vm;
  if (!vm || !op.data) {
    console.warn('[协作-造型] UPDATE 跳过: vm或data缺失');
    return;
  }
  const runtime = vm.runtime;
  const storage = runtime && runtime.storage;
  ...
  if (!storage || !renderer) {
    console.warn('[协作] storage/renderer 不可用，无法更新造型');
    return;
  }
  const costumes = target.sprite?.costumes || [];
  ...
  if (!costume) {
    console.warn(`[协作-造型] UPDATE 跳过: index=${op.index} costume为null`);
    return;
  }
  console.log(
    `[协作-造型] UPDATE target=${target.id} index=${op.index} name=${op.name} oldAssetId=${costume.assetId} newAssetId=${op.assetId} fmt=${op.dataFormat} oldSkinId=${costume.skinId}`
  );
  try {
    const bin = atob(op.data);
    const bytes = new Uint8Array(bin.length);
    ...
    if (fmt === 'svg') {
      ...
      img.onload = () => {
        ...
        try {
          newSkinId = renderer.createBitmapSkin(img, costume.bitmapResolution, [
          ...
          console.error('[协作] 创建 bitmap skin 失败:', e);
        }
        URL.revokeObjectURL(objUrl);
      };
      img.onerror = (e) => {
...
function applySoundAdd(target, op) {
  const vm = target.runtime?.vm || window.vm;
  if (!vm || !op.data) {
    console.warn('[协作-音频] ADD 跳过: vm或data缺失');
    return Promise.resolve();
  }
  const runtime = vm.runtime;
  const storage = runtime && runtime.storage;
  if (!storage) {
    console.warn('[协作] storage 不可用，无法添加音频');
    return Promise.resolve();
  }
  try {
    const bin = atob(op.data);
    ...
    console.log(
      `[协作-音频] ADD target=${target.id} index=${op.index} name=${op.name} assetId=${asset.assetId} fmt=${op.dataFormat} size=${bytes.byteLength}`
    );
    const soundObj = {
      name: op.name,
      assetId: asset.assetId,
    ...
    console.error('[协作] 创建音频 asset 失败:', e);
    return Promise.resolve();
  }
}
...
function applySoundUpdate(target, op) {
  ...
  const vm = target.runtime?.vm || window.vm;
  if (!vm || !op.data) {
    console.warn('[协作-音频] UPDATE 跳过: vm或data缺失');
    return;
  }
  const runtime = vm.runtime;
  const storage = runtime && runtime.storage;
  if (!storage) {
    console.warn('[协作] storage 不可用，无法更新音频');
    return;
  }
  const sounds = target.sprite?.sounds || [];
  ...
  if (!sound) {
    console.warn(`[协作-音频] UPDATE 跳过: index=${op.index} sound为null`);
    return;
  }
  console.log(
    `[协作-音频] UPDATE target=${target.id} index=${op.index} name=${op.name} oldAssetId=${sound.assetId} newAssetId=${op.assetId} fmt=${op.dataFormat}`
  );
  try {
    const bin = atob(op.data);
    const bytes = new Uint8Array(bin.length);
...
async function flush(rtc) {
  ...
  const newOps = Array.from(_pending.values());
  _pending.clear();
  const ops = _def ? _def.concat(newOps) : newOps;
  _def = null;
  ...
  const editIdx = vm.runtime.targets.findIndex(
    (t) => vm.runtime._editingTarget && t.id === vm.runtime._editingTarget.id
  );
  const ws = _Blockly && _Blockly.getMainWorkspace();
  if (ws && ws.isDragging()) {
    for (const op of ops) {
      if (op.targetIndex === editIdx) {
        _def = ops;
        if (!_defRaf)
          _defRaf = requestAnimationFrame(() => {
            _defRaf = null;
            flush(rtc);
          });
        return;
  ...
  if (rtc._updateTimer) {
    clearTimeout(rtc._updateTimer);
    rtc._updateTimer = null;
    rtc.updateProject();
  }
  ...
  try {
    ...
    for (const op of ops) {
      if (!byTarget.has(op.targetIndex)) byTarget.set(op.targetIndex, []);
      byTarget.get(op.targetIndex).push(op);
    }
    for (const [ti, tops] of byTarget) {
      ...
      try {
        ...
        for (const op of tops) {
          switch (op.type) {
            case 'delete':
              applyDelete(target, op, isEditingTarget);
              break;
            case 'move':
              applyMove(target, op, isEditingTarget);
              break;
            case 'create':
              replaceTree(target, op.xml, op.rootId, isEditingTarget);
              break;
            case 'update':
              replaceTree(target, op.xml, op.rootId, isEditingTarget, op.oldRootId);
              break;
            case 'fieldChange':
              applyFieldChange(target, op);
              break;
            case 'mutationChange':
              applyMutationChange(target, op, isEditingTarget);
              break;
            case 'commentSync':
              applyCommentSync(target, op, isEditingTarget);
              break;
            case 'costumeAdd': {
              const p = applyCostumeAdd(target, op);
              if (p) assetPromises.push(p);
              break;
            }
            case 'costumeUpdate':
              applyCostumeUpdate(target, op, rtc);
              break;
            case 'costumeDelete':
              applyCostumeDelete(target, op);
              break;
            case 'soundAdd': {
              const p = applySoundAdd(target, op);
              if (p) assetPromises.push(p);
              break;
            }
            case 'soundUpdate':
              applySoundUpdate(target, op);
              break;
            case 'soundDelete':
              applySoundDelete(target, op);
              break;
          }
        }
        ...
        target.blocks.resumeProjectChanged();
      }
  ...
  return async function handlePeerMessage(peerId, msgs) {
    ...
    switch (data.type) {
      case SERVER_OPCODE.SNAPSHOT: {
        ...
        // 清除加入房间时的超时定时器
        if (rtc._snapshotTimeout) {
          clearTimeout(rtc._snapshotTimeout);
        ...
        const ua = requestProjectUpload(oldSt);
        if (ua) ReduxStore.dispatch(ua);
        ...
        const cleanupSnapshotState = (exit) => {
          ...
          if (exit) {
            try {
              ...
              console.error('[协作] snapshot 失败后 exit 异常:', e);
            }
          }
        };
        try {
          ...
          try {
            ...
            console.error('[协作] renderer.draw 失败:', e);
          }
          ok = true;
          ...
          const d = onLoadedProject(ls, false, ok);
          if (d) ReduxStore.dispatch(d);
        }
        setTimeout(async () => {
          ...
          if (rtc._deferredPeerMessages.length > 0) {
            ...
            for (const d of deferred) {
              ...
              try {
                await handlePeerMessage(d.peerId, d.msgs);
              } catch (e) {
                console.error('[协作] 重放暂存消息失败:', e);
              }
            }
          }
      ...
      case SERVER_OPCODE.POINTER: {
        ...
        const pc = getPointerContainer(true);
        if (!pc) break;
        const ws2 = getWS();
        if (!ws2) break;
        ...
        if (old) {
          ...
          pc.appendChild(el);
          const te = el.querySelector('.sa-collab-name-text'),
            be = el.querySelector('.sa-collab-name-bg');
          if (te && be) {
            const b = te.getBBox();
            be.setAttribute('width', b.width + 20);
          }
          setupScaleObserver();
        }
        break;
      ...
      case SERVER_OPCODE.BLOCK_FIELD_CHANGE:
        if (data.targetIndex !== undefined && data.fields && data.rootId)
          enqueue(
            { type: 'fieldChange', targetIndex: data.targetIndex, rootId: data.rootId, fields: data.fields },
            rtc
          );
        break;
      case SERVER_OPCODE.BLOCK_MUTATION_CHANGE:
        if (data.targetIndex !== undefined && data.rootId && data.mutation)
          enqueue(
            { type: 'mutationChange', targetIndex: data.targetIndex, rootId: data.rootId, mutation: data.mutation },
            rtc
          );
        break;
      // ── Comment sync ──
      ...
      case SERVER_OPCODE.SPRITE_ADD:
        if (data.spriteData) {
          ...
          try {
            ...
            try {
              const b2 = atob(data.spriteData);
              const bytes2 = new Uint8Array(b2.length);
      ...
      case SERVER_OPCODE.PING:
        sendToPeer(peerId, JSON.stringify({ type: SERVER_OPCODE.PONG }));
        break;
