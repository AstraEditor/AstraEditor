...
export class RTCServer {
  constructor({ msg, console, updateTipText, vm, onPeerMessage, onStateChange, onPeerLeft }) {
    this._msg = msg;
  ...
  _bindPointerToCurrentWorkspace() {
    ...
    if (this._boundWorkspace && this.boundMouseMoveHandler) {
      try {
        this._boundWorkspace.removeEventListener('mousemove', this.boundMouseMoveHandler);
      } catch {
        /* workspace已dispose */
      }
    }
  ...
  _heartbeatTick() {
    ...
    for (const [peerId, lastSeen] of this._peerLastSeen) {
      ...
      if (now - lastSeen > RTCServer.HEARTBEAT_TIMEOUT) {
        this._handlePeerTimeout(peerId);
        continue;
      }
  ...
  _queueSend(peerId, messages) {
    ...
    if (queue.length + messages.length > RTCServer.PENDING_LIMIT) {
      return false;
    }
    queue.push(...messages);
  ...
  _messageToChunks(data) {
    ...
    for (let offset = 0, idx = 0; offset < data.length; offset += RTCServer.CHUNK_SIZE, idx++) {
      chunks.push(
        JSON.stringify({ type: '_chunk', id, idx, total, data: data.slice(offset, offset + RTCServer.CHUNK_SIZE) })
      );
    }
    return chunks;
  }
  ...
  _scheduleDrain(peerId, delay = 20) {
    ...
    const timer = setTimeout(() => {
      this._sendDrainTimers.delete(peerId);
      this._drainSendQueue(peerId);
    }, delay);
  ...
  _drainSendQueue(peerId) {
    ...
    try {
      ...
      this._scheduleDrain(peerId, 250);
      return;
    }
    if (queue.length === 0) {
  ...
  async login(mode = 'reg', serverUrl = 'localhost:1832', roomId = '') {
    ...
    try {
      ...
      if (this._loginSession !== loginSession) {
        this._console.log('[协作] login 被 abort，中止');
        return;
      }
      ...
      this._server.onmessage = (msgs) => {
        try {
          Promise.resolve(this._handleServerMessage(JSON.parse(msgs.data))).catch((e) =>
            this._console.error('[协作] 处理服务器消息异常:', e)
          );
        } catch (e) {
        ...
        this._vm.loadProject = async (...args) => {
          if (this.state.clientId && !this.shouldIgnoreUpdate()) {
            ...
            try {
              ...
              this._console.error('[协作] exit 异常:', e);
            }
          }
  ...
  async updateProject() {
    ...
    for (let i = 0; i < liveTargets.length; i++) {
      ...
      if (!this._scriptBlockCache.has(target.id) && !oldIds.has(target.id)) {
        const nc = new Map();
        this._scriptBlockCache.set(target.id, nc);
        if (target.blocks && target.blocks._scripts) {
          for (const rid of target.blocks._scripts) {
            try {
              nc.set(rid, target.blocks.blockToXML(rid, target.comments || {}));
            } catch (e) {}
          }
        }
      }
    ...
    for (let targetIndex = 0; targetIndex < liveTargets.length; targetIndex++) {
        ...
        for (const newRid of addedRootIds) {
          ...
          try {
            ...
            if (
              isFinite(newX) &&
              isFinite(newY) &&
              Math.abs(oldX - newX) < 5 &&
              Math.abs(oldY - newY) < 5 &&
              oldOpcode === newOpcode
            ) {
      ...
      for (const [oldRid, { newRid, newXml }] of replacements) {
        waitForBroadcast.push({
          type: SERVER_OPCODE.BLOCK_UPDATE,
          targetIndex,
          rootId: newRid,
          oldRootId: oldRid,
          xml: newXml
      ...
      for (const rid of addedRootIds) {
        ...
        try {
          ...
          this._console.error(`[协作] 序列化脚本 ${rid} 失败:`, e);
        }
      }
      ...
      for (const rid of commonIds) {
        try {
          ...
          this._console.error(`[协作] 比较脚本 ${rid} 失败:`, e);
        }
      }
      ...
      if (!this._costumeCache.has(targetId)) {
        ...
        this._console.log(
          `[协作-造型] 初始化缓存 target=${targetId} count=${costumes.length} assetIds=[${costumes.map((c) => c.assetId).join(',')}]`
        );
      }
      ...
      if (JSON.stringify(costumeSnapshot) !== JSON.stringify(cachedCostumes)) {
        this._console.log(
          `[协作-造型] 检测到变更 target=${targetId} live=${costumes.length} cached=${cachedCostumes.length}`
        );
        ...
        for (let i = 0; i < maxLen; i++) {
          ...
          if (!live && cached) {
            ...
            if (data) {
              ...
              waitForBroadcast.push({
                type: SERVER_OPCODE.COSTUME_ADD,
                targetIndex,
                index: i,
                name: live.name,
                assetId: live.assetId,
                dataFormat: live.dataFormat,
                bitmapResolution: live.bitmapResolution,
                rotationCenterX: live.rotationCenterX,
                rotationCenterY: live.rotationCenterY,
                data: btoa(binary)
              });
              this._console.log(
                `[协作-造型] ADD index=${i} name=${live.name} assetId=${live.assetId} fmt=${live.dataFormat} size=${bytes.byteLength}`
              );
            } else {
              this._console.warn(
                `[协作-造型] ADD index=${i} name=${live.name} 但 asset.data 为空，跳过！assetId=${live.assetId} asset=${!!live.asset}`
              );
            }
          } else if (live && cached) {
            ...
            const cachedKey = {
              ...
              bitmapResolution: cached.bitmapResolution,
              rotationCenterX: cached.rotationCenterX,
              rotationCenterY: cached.rotationCenterY
            };
            if (JSON.stringify(liveKey) !== JSON.stringify(cachedKey)) {
              ...
              if (data) {
                ...
                for (let bi = 0; bi < bytes.byteLength; bi++) binary += String.fromCharCode(bytes[bi]);
                waitForBroadcast.push({
                  type: SERVER_OPCODE.COSTUME_UPDATE,
                  targetIndex,
                  index: i,
                  name: live.name,
                  assetId: live.assetId,
                ...
                this._console.log(
                  `[协作-造型] UPDATE index=${i} name=${live.name} oldAssetId=${cached.assetId} newAssetId=${live.assetId} fmt=${live.dataFormat} size=${bytes.byteLength}`
                );
              } else {
                this._console.warn(
                  `[协作-造型] UPDATE index=${i} name=${live.name} 但 asset.data 为空，跳过！assetId=${live.assetId} asset=${!!live.asset}`
                );
              }
            }
          }
      ...
      if (!this._soundCache.has(targetId)) {
        ...
        this._console.log(
          `[协作-音频] 初始化缓存 target=${targetId} count=${sounds.length} assetIds=[${sounds.map((s) => s.assetId).join(',')}]`
        );
      }
      const cachedSounds = this._soundCache.get(targetId);
      if (JSON.stringify(soundSnapshot) !== JSON.stringify(cachedSounds)) {
        this._console.log(
          `[协作-音频] 检测到变更 target=${targetId} live=${sounds.length} cached=${cachedSounds.length}`
        );
        ...
        for (let i = 0; i < maxLen; i++) {
          ...
          if (!live && cached) {
            ...
            if (data) {
              ...
              this._console.log(
                `[协作-音频] ADD index=${i} name=${live.name} assetId=${live.assetId} fmt=${live.dataFormat} size=${bytes.byteLength}`
              );
            } else {
              this._console.warn(
                `[协作-音频] ADD index=${i} name=${live.name} 但 asset.data 为空，跳过！assetId=${live.assetId} asset=${!!live.asset}`
              );
            }
          } else if (live && cached) {
            const liveKey = {
            ...
            if (JSON.stringify(liveKey) !== JSON.stringify(cachedKey)) {
              ...
              if (data) {
                ...
                for (let bi = 0; bi < bytes.byteLength; bi++) binary += String.fromCharCode(bytes[bi]);
                waitForBroadcast.push({
                  type: SERVER_OPCODE.SOUND_UPDATE,
                  targetIndex,
                  index: i,
                  name: live.name,
                  assetId: live.assetId,
                ...
                this._console.log(
                  `[协作-音频] UPDATE index=${i} name=${live.name} oldAssetId=${cached.assetId} newAssetId=${live.assetId} fmt=${live.dataFormat} size=${bytes.byteLength}`
                );
              } else {
                this._console.warn(
                  `[协作-音频] UPDATE index=${i} name=${live.name} 但 asset.data 为空，跳过！assetId=${live.assetId} asset=${!!live.asset}`
                );
              }
            }
          }
    ...
    for (const item of waitForBroadcast) {
      ...
      if (item.type === SERVER_OPCODE.BLOCK_DELETE) {
        for (const rid of item.rootIds)
          opMap.set(`${item.targetIndex}:block:${rid}`, {
            type: SERVER_OPCODE.BLOCK_DELETE,
        ...
        item.type === SERVER_OPCODE.COSTUME_ADD ||
        item.type === SERVER_OPCODE.COSTUME_DELETE ||
        item.type === SERVER_OPCODE.COSTUME_UPDATE
      ) {
        key = `${item.targetIndex}:costume:${item.index}`;
      } else if (
        item.type === SERVER_OPCODE.SOUND_ADD ||
        item.type === SERVER_OPCODE.SOUND_DELETE ||
        item.type === SERVER_OPCODE.SOUND_UPDATE
      ) {
        key = `${item.targetIndex}:sound:${item.index}`;
      } else if (
        item.type === SERVER_OPCODE.VARIABLE_ADD ||
        item.type === SERVER_OPCODE.VARIABLE_DELETE ||
        item.type === SERVER_OPCODE.VARIABLE_RENAME
      ) {
    ...
    if (finalMessages.length > 0) {
      const msgSummary = finalMessages
        .map((m) => {
          ...
          if (
            m.type === SERVER_OPCODE.COSTUME_ADD ||
            m.type === SERVER_OPCODE.COSTUME_DELETE ||
            m.type === SERVER_OPCODE.COSTUME_UPDATE
          )
            return `${m.type}:${m.name}`;
          if (
            m.type === SERVER_OPCODE.SOUND_ADD ||
            m.type === SERVER_OPCODE.SOUND_DELETE ||
            m.type === SERVER_OPCODE.SOUND_UPDATE
          )
            return `${m.type}:${m.name}`;
          if (
            m.type === SERVER_OPCODE.VARIABLE_ADD ||
            m.type === SERVER_OPCODE.VARIABLE_DELETE ||
            m.type === SERVER_OPCODE.VARIABLE_RENAME
          )
            return `${m.type}:${m.name || m.varId}`;
      ...
      for (const item of finalMessages) {
        if (item.type === SERVER_OPCODE.SPRITE_ADD) {
          try {
            ...
            this.broadcastToPeers(
              JSON.stringify({ type: SERVER_OPCODE.SPRITE_ADD, targetIndex: item.index, spriteData: base64Data })
            );
          } catch (e) {
    ...
    setTimeout(() => {
      try {
        ...
        if (this._localEditState) {
          const lockId =
            this._localEditState.type === 'comment' ? String(this._localEditState.index) : this._localEditState.id;
          this.broadcastToPeers(
            JSON.stringify({
  ...
  _sendPointer(e) {
    ...
    this.broadcastToPeers(
      JSON.stringify({
        type: SERVER_OPCODE.POINTER,
        id: this.state.clientId,
        workspaceIndex: this.editingTargetIndex,
        name: this.getUserName(),
        position: { x: canvasCoord.x, y: canvasCoord.y },
  ...
  _broadcastChatMessage(text) {
    this.broadcastToPeers(
      JSON.stringify({
        ...
        id: this.state.clientId,
        themeColor:
          getComputedStyle(document.documentElement).getPropertyValue('--looks-secondary').trim() || '#0099ff',
        name: this.getUserName(),
    ...
    // 同时更新本地气泡
    if (text !== null) {
      this._updateLocalChatBubble(this._lastPointerPos.x, this._lastPointerPos.y, text);
    }
  }
  ...
  _setupEditLockListeners() {
    ...
    const isCommentTextarea = (el) => {
      return el?.classList?.contains('scratchCommentTextarea') || el?.classList?.contains('blocklyCommentTextarea');
    };
    const onFocusIn = (e) => {
    ...
    if (wd && typeof wd.hide === 'function' && typeof wd.show === 'function') {
      const origHide = wd.hide,
        origShow = wd.show;
      const self = this;
      wd.hide = function (...args) {
        origHide.apply(this, args);
        self._detectAndBroadcastEditState();
      };
      wd.show = function (...args) {
  ...
  _detectAndBroadcastEditState() {
    ...
    const isAnyCommentTextarea =
      active &&
      (active.classList.contains('scratchCommentTextarea') || active.classList.contains('blocklyCommentTextarea'));
    ...
    const oldKey = this._localEditState
      ...
      : null;
    const newKey = newState ? `${newState.type}:${newState.type === 'comment' ? newState.index : newState.id}` : null;
    ...
    // 发送新锁定
    if (newState) {
      const lockId = newState.type === 'comment' ? String(newState.index) : newState.id;
      this.broadcastToPeers(
        JSON.stringify({
    ...
    try {
      ...
      for (const listener of this._editLockListeners) {
        if (listener.type === 'mutation_observer' && listener.observer) {
          ...
          (listener.type === 'widgetdiv_hide_patch' || listener.type === 'widgetdiv_show_patch') &&
          listener.original
        ) {
      ...
      if (this._commentRetryTimer) {
        clearTimeout(this._commentRetryTimer);
        this._commentRetryTimer = null;
      }
      if (this._pendingUnlockTimer) {
        clearTimeout(this._pendingUnlockTimer);
      ...
      if (this._boundProjectChanged) {
        this._vm.runtime.off('PROJECT_CHANGED', this._boundProjectChanged);
        this._boundProjectChanged = null;
      }
      if (this._boundExtensionRemoved) {
        this._vm.runtime.off('EXTENSION_REMOVED', this._boundExtensionRemoved);
        this._boundExtensionRemoved = null;
      }
      if (this._boundTargetsUpdate) {
        this._vm.off('targetsUpdate', this._boundTargetsUpdate);
        this._boundTargetsUpdate = null;
      }
      this._loggingIn = false;
      ...
      if (this._server) {
        try {
          this._server.send(JSON.stringify({ type: 'exit', clientId: this.state.clientId }));
        } catch (e) {}
        try {
          this._server.close();
        } catch (e) {}
  ...
  sendToPeer(peerId, data, dropIfBuffered) {
    ...
    if (dropIfBuffered && (channel.bufferedAmount > RTCServer.BUFFER_HIGH_WATER || this._sendQueues.has(peerId)))
      return false;
    try {
      if (
        data.length <= RTCServer.CHUNK_SIZE &&
        !this._sendQueues.has(peerId) &&
        channel.bufferedAmount < RTCServer.BUFFER_HIGH_WATER
      ) {
        channel.send(data);
      } else {
    ...
    switch (data.type) {
      case 'connection':
        ...
        data.existingPeers.forEach((peer) => {
          if (peer.cid === this.state.clientId && peer.owner) this._isHost = true;
        });
      ...
      case 'peer-joined':
        ...
        if (this._isHost) {
          ...
          try {
            const extensions = [];
            const extURLs = this._vm.extensionManager.getExtensionURLs
              ? this._vm.extensionManager.getExtensionURLs()
              : {};
