export default async function () {
  ...
  (function () {
          ...
          switch (cmd) {
            case 'BLOCK': {
              ...
              if (parent.type === 'statement') {
                ...
                const emptySubstack = parent.element.querySelector(
    ...
    let config = {
      ...
      headers: JSON.parse(
        localStorage.getItem(STORAGE_KEYS.HEADERS) || '[{"key":"Authorization","value":"Bearer {{apiKey}}"}]'
      ),
      bodyTemplate:
        localStorage.getItem(STORAGE_KEYS.BODY_TEMPLATE) ||
    ...
    function extractScratchDSL(text) {
      ...
      if (
        text.includes('BLOCK event_whenflagclicked') ||
        text.includes('BLOCK motion_') ||
        text.includes('BLOCK looks_')
      ) {
        return text.trim();
      }
      return null;
      ...
      function formatText(text) {
        ...
        text = text
          ...
          .replace(
            /```[\s\S]*?```/g,
            (match) =>
              `<pre style="background:rgba(0,0,0,0.05); padding:10px; border-radius:5px; overflow-x:auto; font-size:12px;">${match.replace(/```/g, '')}</pre>`
          );
