        ...
        /*
{
    ...
    "steps": [
      ...
      if (isNew)
        document
          ...
          .parentNode.insertBefore(modeTab, document.querySelector('[class*=sa-todo-content]'));
  ...
  const getTodoListContent = () => {
    try {
      return JSON.parse(
        getTodoList()['text'].split(POINT)[1]
        /**
         * 我们的格式是:
         *
         * xxx
  ...
  AddToBar(addon, {
    ...
    onClick: () => {
      const { backdrop, container, content, closeButton, remove } = addon.tab.createModal(
        msg('title', { project: PROJECT_NAME.toString() }),
        {
          isOpen: true,
          useEditorClasses: true
        }
