...
test('detect motionAmount between frames', (t) => {
  ...
  return readFrames().then((frames) => {
    ...
    for (const [frame1, frame2] of framePairs) {
      ...
      detect.analyzeFrame();
      t.ok(detect.motionAmount > 10, `frame pair ${index + 1} has motion ${detect.motionAmount} over threshold (10)`);
      index += 1;
    }
  ...
  return readFrames().then((frames) => {
    ...
    for (const {
      ...
      t.ok(
        ...
        `frame pair ${index + 1} is ${detect.motionDirection.toFixed(0)} ` + `degrees and close to ${direction} degrees`
      );
      index += 1;
    }
