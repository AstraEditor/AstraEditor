...
class ExtensionManager {
  ...
  _normalizeExtensionURL(extensionURL) {
    ...
    if (/^\/\/.+/.test(url)) {
      ...
      url = `https://${url}`;
    } else if (!/^[a-z][a-z0-9+.-]*:/i.test(url) && /^[a-z0-9.-]+\.[a-z]{2,}(?:\/|$)/i.test(url)) {
      url = `https://${url}`;
    }
    ...
    for (const candidateURL of candidates) {
      ...
      if (!this._isValidExtensionURL(candidateURL)) {
        ...
        document.dispatchEvent(
          new CustomEvent('loadExtensionDone', {
            detail: {
              state: 'error',
              info: `Invalid extension URL: ${candidateURL}`
            }
          })
        );
        continue;
  ...
  async _loadCustomExtensionURL(extensionURL, Trust) {
    ...
    try {
      ...
      if (sandboxMode === 'worker') {
        ...
        ExtensionWorker = (
          await import(/* webpackChunkName: "iframe-extension-worker" */ './tw-iframe-extension-worker')
        ).default;
      } else {
        ...
        const callBlockFunc = (() => {
          if (dispatch._isRemoteService(serviceName)) {
            return (args, util, realBlockInfo) =>
              dispatch.call(serviceName, funcName, args, util, realBlockInfo).then((result) => {
  ...
  static _isExtensionOpcode(block, extensionId) {
    return block && typeof block.opcode === 'string' && block.opcode.startsWith(`${extensionId}_`);
  }
