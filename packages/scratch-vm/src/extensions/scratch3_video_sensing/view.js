  ...
  draw() {
    ...
    if (this.output === OUTPUT.XYT_CELL) {
      ...
      this._eachCell(WINSIZE + 1, WINSIZE + 1, wmax, hmax, winStep, winStep, (eachAddress) => {
        ...
        eachAddress((address) => {
          buffer[address] =
            (0xff << 24) + (((((C1 * 0x7f) | 0) + 0x80) << 8) & 0xff00) + (((((C2 * 0x7f) | 0) + 0x80) << 0) & 0xff);
        });
      });
    }
    ...
    if (this.output === OUTPUT.T_CELL) {
      ...
      this._eachAddress(1, 1, WIDTH - 1, HEIGHT - 1, (address) => {
        const { gradX, gradY, gradT } = this._grads(address);
        buffer[address] =
          (0xff << 24) + (((Math.sqrt(gradY * gradT) * 0x0f) & 0xff) << 8) + ((Math.sqrt(gradX * gradT) * 0x0f) & 0xff);
      });
    }
    if (this.output === OUTPUT.C_CELL) {
      ...
      this._eachAddress(1, 1, WIDTH - 1, HEIGHT - 1, (address) => {
        const { gradX, gradY } = this._grads(address);
        buffer[address] =
          (0xff << 24) + (((gradX * gradY) & 0xff) << 16) + (((gradY * gradY) & 0xff) << 8) + ((gradX * gradX) & 0xff);
      });
    }
      ...
      this._eachAddress(1, 1, WIDTH - 1, HEIGHT - 1, (address) => {
        ...
        buffer[address] =
          ...
          (inRange && amount > THRESHOLD
            ? (((((v / winStep + 1) / 2) * 0xff) << 8) & 0xff00) + (((((u / winStep + 1) / 2) * 0xff) << 0) & 0xff)
            : 0x8080);
      });
