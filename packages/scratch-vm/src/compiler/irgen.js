...
class ScriptTreeGenerator {
  /**
   * @param {{ target: any; blockContainer: any; }} thread
   */
  constructor(thread) {
    /** @private */
    this.thread = thread;
  ...
  getBlockInfo(fullOpcode) {
    ...
    const categoryInfo = this.runtime._blockInfo.find((/** @type {{ id: string; }} */ ci) => ci.id === category);
    if (!categoryInfo) {
      return null;
    }
    const blockInfo = categoryInfo.blocks.find(
      (/** @type {{ info: { opcode: string; }; }} */ b) => b.info.opcode === opcode
    );
    if (!blockInfo) {
      return null;
    }
  ...
  /**
   * @param {string | number | boolean | null} constant
   */
  ...
  descendInput(block, preserveStrings = false) {
    ...
    switch (block.opcode) {
      ...
      case 'operator_mathop': {
        ...
        switch (operator) {
          case 'abs':
            return new IntermediateInput(InputOpcode.OP_ABS, InputType.NUMBER_POS | InputType.NUMBER_ZERO, { value });
          case 'floor':
            return new IntermediateInput(InputOpcode.OP_FLOOR, InputType.NUMBER_INT | InputType.NUMBER_INF, { value });
          case 'ceiling':
      ...
      case 'sensing_current':
        switch (block.fields.CURRENTMENU.value.toLowerCase()) {
          ...
          case 'month':
            return new IntermediateInput(InputOpcode.SENSING_TIME_MONTH, InputType.NUMBER_POS_INT);
          case 'date':
            return new IntermediateInput(InputOpcode.SENSING_TIME_DATE, InputType.NUMBER_POS_INT);
          case 'dayofweek':
            return new IntermediateInput(InputOpcode.SENSING_TIME_WEEKDAY, InputType.NUMBER_POS_INT);
          case 'hour':
            return new IntermediateInput(
          ...
          switch (property) {
            ...
            case 'costume name':
              return new IntermediateInput(
                ...
                InputType.STRING | InputType.NUMBER_ZERO,
                { object }
              );
            case 'size':
              return new IntermediateInput(InputOpcode.SENSING_OF_SIZE, InputType.NUMBER_POS | InputType.NUMBER_ZERO, {
                object
              });
          }
        }
  ...
  walkHat(hatBlock) {
    ...
    if (this.thread.stackClick) {
      ...
      if (opcodeFunction) {
        return new IntermediateStack([this.descendCompatLayerStack(hatBlock), ...this.walkStack(nextBlock).blocks]);
      }
...
class IRGenerator {
  /**
   * @param {{ blockContainer: any; }} thread
   */
  constructor(thread) {
    this.thread = thread;
    this.blocks = thread.blockContainer;
