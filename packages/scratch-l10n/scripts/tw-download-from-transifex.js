...
const processAddons = (translations) => {
  ...
  if (fs.existsSync(scratchGuiPath)) {
    ...
    fs.writeFileSync(
      pathUtil.join(scratchGuiPath, 'src/addons/settings/translations.json'),
...
const processDesktopWeb = (translations) => {
  ...
  if (fs.existsSync(desktopPath)) {
    ...
    const newContent = oldContent.replace(
      /\/\*===\*\/[\s\S]+\/\*===\*\//m,
      `/*===*/${semiPrettyPrintJSON(translations)}/*===*/`
    );
...
const processPackager = (translations) => {
  ...
  if (fs.existsSync(packagerPath)) {
    ...
    fs.writeFileSync(pathUtil.join(packagerPath, 'locale-names.json'), JSON.stringify(localeNames, null, 4));
  }
};
...
const processStoreListings = (translations) => {
  ...
  if (fs.existsSync(desktopPath)) {
    ...
    fs.writeFileSync(pathUtil.join(desktopPath, 'store-listings/imported.json'), JSON.stringify(translations, null, 4));
  }
};

(async () => {
  const [guiMessages, addonMessages, desktopMessages, desktopWebMessages, packagerMessages, storeListingMessages] =
    await Promise.all([
