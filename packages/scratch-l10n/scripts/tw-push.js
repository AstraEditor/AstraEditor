...
const parseSourceGuiMessages = () => {
  const reactTranslationFiles = recursiveReadDirectory(scratchGuiTranslations).filter((file) => file.endsWith('.json'));
...
fs.writeFileSync(pathUtil.join(__dirname, 'tw-all-used-ids.json'), JSON.stringify(allMessageIds, null, 4));
